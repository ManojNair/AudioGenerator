...
class SSMLPlugin:
    ...
    @staticmethod
    def ssml_to_speech(ssml_input: str, output_filename: str = None) -> str:
        """
        ...
        try:
            ...
            filename = output_filename if output_filename else OUTPUT_FILENAME
            ...
            if result.reason == ResultReason.SynthesizingAudioCompleted:
                wait_time = 0
                while not (os.path.exists(filename) and os.path.getsize(filename) > 0) and wait_time < 5:
                    time.sleep(0.5); wait_time += 0.5
                if not (os.path.exists(filename) and os.path.getsize(filename) > 0):
                    return f"Error: Output file {filename} is missing or empty after synthesis completed."
                else:
                    print(f"SSML synthesis successful. File saved to {filename} ({os.path.getsize(filename)} bytes).")
                    return f"Speech synthesis successful. File saved at: {filename}"
                ...
                if os.path.exists(filename):
                    ...
                    except OSError as remove_err: print(f"Warning: Could not remove partially created file {filename}: {remove_err}")
                ...
                if os.path.exists(filename):
                    try: os.remove(filename)
                    except OSError as remove_err: print(f"Warning: Could not remove potentially invalid file {filename}: {remove_err}")
                return error_message
            ...
            if os.path.exists(filename):
                try: os.remove(filename)
                except OSError as remove_err: print(f"Warning: Could not remove file {filename} during exception handling: {remove_err}")
            return f"Error during SSML to speech processing: {str(e)}"

async def analyze_with_llm(kernel: Kernel, prompt: str, context: str) -> str:
    """Helper function to analyze content using the LLM"""
    try:
        # Create arguments for the function - updated for v1.36.1
        args = KernelArguments(context=context)
        
        # Create a prompt function that the kernel can invoke
        prompt_function = kernel.add_function(
            prompt=prompt,
            function_name="analysis_function",
            plugin_name="analysis_plugin"
        )
        
        # Use the kernel's invoke method with the created function
        ...
        # Properly extract text content from the result
        if result and hasattr(result, 'value'):
            content = result.value
            print(f"DEBUG: Content type: {type(content)}")
            print(f"DEBUG: Content dir: {[attr for attr in dir(content) if not attr.startswith('_')]}")
        else:
            content = str(result)
            print(f"DEBUG: Using str(result), content type: {type(content)}")
        
        # Handle list of ChatMessageContent objects
        if isinstance(content, list) and len(content) > 0:
            # Get the first ChatMessageContent object
            first_content = content[0]
            print(f"DEBUG: First content type: {type(first_content)}")
            print(f"DEBUG: First content dir: {[attr for attr in dir(first_content) if not attr.startswith('_')]}")
            
            # Extract the actual text content
            if hasattr(first_content, 'content') and hasattr(first_content.content, 'content'):
                # This is a ChatMessageContent object, extract the inner content
                final_content = str(first_content.content.content)
                print(f"DEBUG: Extracted from first_content.content.content: {final_content[:100]}...")
                return final_content
            elif hasattr(first_content, 'content'):
                # This might be a different type of content object
                final_content = str(first_content.content)
                print(f"DEBUG: Extracted from first_content.content: {final_content[:100]}...")
                return final_content
            else:
                # Regular string content
                final_content = str(first_content)
                print(f"DEBUG: Using str(first_content): {final_content[:100]}...")
                return final_content
        # If the content is a ChatMessageContent object, extract the actual text
        elif hasattr(content, 'content') and hasattr(content.content, 'content'):
            # This is a ChatMessageContent object, extract the inner content
            final_content = str(content.content.content)
            print(f"DEBUG: Extracted from content.content.content: {final_content[:100]}...")
            return final_content
            ...
            print(f"DEBUG: Using str(content): {final_content[:100]}...")
            return final_content
...
async def main():
    ...
    # Add Azure OpenAI chat service - updated for v1.36.1
    ...
    # Updated service registration for v1.36.1
    kernel.add_service(chat_service)
    
    print("\nTesting Azure OpenAI connection...")
    try:
        # Test the connection - updated for v1.36.1
        test_prompt = "Hello! This is a test message."
        test_function = kernel.add_function(
            prompt=test_prompt,
            function_name="test_function",
            plugin_name="test_plugin"
        )
        
        test_args = KernelArguments(context="Hello! This is a test message.")
        response = await kernel.invoke(test_function, test_args)
    ...
    try:
        ...
        """
        business_analysis = await analyze_with_llm(kernel, business_analysis_prompt, image_description)
        ...
        **Formatting Requirements:**
        ...
        """
        podcast_script = await analyze_with_llm(kernel, podcast_script_prompt, business_analysis)
        ...
        # Debug: Print the first 500 characters of the SSML to identify issues
        print(f"\n--- SSML Debug (first 500 chars) ---")
        print(formatted_ssml[:500])
        print(f"--- End SSML Debug ---\n")
        
        # Step 5: Convert to speech
        print("Step 5: Converting to speech...")
        speech_result = ssml_plugin.ssml_to_speech(formatted_ssml, OUTPUT_FILENAME)
